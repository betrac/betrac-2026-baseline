...
# Submit Qwen3-Omni-30B-A3B-Thinking inference array job + combine job for Exp0004
# Run from: experiments/Exp0004-qwen3-thinking/
#
# Usage:
...
#   bash slurm/submit_omni.sh --time=03:00:00        # override per-task walltime
#
# Environment variables (all optional):
#   MODEL_ID          HuggingFace model ID           (default: Qwen/Qwen3-Omni-30B-A3B-Thinking)
#   MODEL_SHORT       Short name for output dir      (default: qwen3-omni-30b-thinking)
...
#   THINKING          Enable thinking mode           (default: 1)
...
MODEL_SHORT="${MODEL_SHORT:-qwen3-omni-30b-thinking}"
...
if [ ! -f "slurm/run_omni_hf.slurm" ]; then
    echo "ERROR: Must run from experiments/Exp0004-qwen3-thinking/"
    echo "  cd $(cd ../.. && pwd)/experiments/Exp0004-qwen3-thinking"
...
# ---------------------------------------------------------------------------
echo "=========================================="
echo "Exp0004 — Qwen3-Omni-30B Thinking Submission"
echo "=========================================="
...
ARRAY_EXPORT+=",FLASH_ATTN=${FLASH_ATTN},THINKING=${THINKING},SAMPLES_PER_TASK=${SAMPLES_PER_TASK}"
...
echo "Monitor with:"
echo "  squeue -u \$USER"
echo "  tail -f slurm/logs-slurm/slurm-Exp0004-omni-qwen3-thinking_${ARRAY_JOB_ID}_<TASK>.out"
