...
# Submit Qwen2.5-Omni-3B inference array job + combine job for Exp0001
# Run from: experiments/Exp0001-qwen25-3b/
#
# Usage:
...
#   bash slurm/submit_omni.sh --time=01:00:00        # override per-task walltime
#
# Environment variables (all optional):
#   MODEL_ID          HuggingFace model ID           (default: Qwen/Qwen2.5-Omni-3B)
#   MODEL_SHORT       Short name for output dir      (default: qwen25-omni-3b)
...
# ---------------------------------------------------------------------------
MODEL_ID="${MODEL_ID:-Qwen/Qwen2.5-Omni-3B}"
MODEL_SHORT="${MODEL_SHORT:-qwen25-omni-3b}"
...
if [ ! -f "slurm/run_omni_hf.slurm" ]; then
    echo "ERROR: Must run from experiments/Exp0001-qwen25-3b/"
    echo "  cd $(cd ../.. && pwd)/experiments/Exp0001-qwen25-3b"
...
# ---------------------------------------------------------------------------
# Count samples (use TOTAL env var to skip slow HF streaming count)
# ---------------------------------------------------------------------------
SAMPLE_COUNT="${TOTAL:-}"
...
# ---------------------------------------------------------------------------
echo "=========================================="
echo "Exp0001 — Qwen2.5-Omni-3B Submission"
echo "=========================================="
...
echo "Monitor with:"
echo "  squeue -u \$USER"
echo "  tail -f slurm/logs-slurm/slurm-Exp0001-omni-qwen25-3b_${ARRAY_JOB_ID}_<TASK>.out"
